...
<div class="Men">
    <div class="categories d-none d-lg-flex">
        ...
        <RouterLink to="/footwear" class="category">Footwear</RouterLink>
        <RouterLink to="/accessories" class="category active">Accessories</RouterLink>
...
<div class="overlay d-none" id="succesMsg">
    ...
    <!-- ******** Database Products of accessories category ******** -->
    
    <div class="products" id="details">
        <div class="card" v-if="!this.accessories.message" v-for="product in accessories.slice((currentPage - 1) * itemsPerPage , currentPage * itemsPerPage)">
        ...
        <h1 v-if="this.accessories.message" class="text-center">{{ accessories.message }}</h1>
    </div>
</div>
    <!-- ******** Database Products of accessories category end ******** -->
...
computed : {
    totalPages(){ // this calculates the number of pages that the products will be showed in 
        return Math.ceil(this.accessories.length / this.itemsPerPage)
    },
    authenticated(){ // checks if the user is authenticated or not
        this.token = this.$store.state.token
        return this.$store.state.authenticated
    }
},
methods: {
    nextPage(){  // shows more products of the next page
    ...
    addToCart(id){ // add product to the cart if user is authenticated
    ...
    ProductDetails(id){ // show product details 
        ...
        axios.get('http://127.0.0.1:8000/api/proudcts/'+ id).then((response)=> {
            ...
            this.$store.state.path = 'accessories'
        }).catch(function(error) {
            console.log(error.response.data.message);
        });
    },
...
beforeMount() {
    // an api that gets the accessories products 
    axios.get("http://127.0.0.1:8000/api/accessories").then((response)=> {
            if(response.data.message){
                this.accessories = response.data.message;
            }
            else{
                this.accessories = response.data.products;
            }
        })
},
mounted() {
