...
<div class="Men">
    <div class="categories d-none d-lg-flex">
        <h2>Categories</h2>
        <hr class="categHr2">
        <RouterLink to="/all" class="category active">All</RouterLink>
...
<div class="overlay d-none" id="succesMsg">
    ...
    <!-- ******** Database Products of all categories ******** -->
     
    <div class="products" id="details">
        <div class="card" v-if="this.allProducts" v-for="product in allProducts.slice((currentPage - 1) * itemsPerPage , currentPage * itemsPerPage)">
        ...
        <h1 v-if="!this.allProducts" class="text-center">There is no products yet</h1>
    </div>
</div>
    <!-- ******** Database Products end ******** -->
...
methods: {
    ...
    ProductDetails(id){
        ...
        axios.get('http://127.0.0.1:8000/api/proudcts/'+ id).then((response)=> {
            ...
            this.$store.state.path = 'All Products'
        }).catch(function(error) {
            console.log(error.response.data.message);
        });
    },
