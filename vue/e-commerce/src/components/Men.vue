...
<div class="Men">
    <div class="categories d-none d-lg-flex">
        ...
        <RouterLink to="/Men" class="category active">Men</RouterLink>
...
<div class="overlay d-none" id="succesMsg">
    ...
    <div class="products" id="details">
        <div class="card" v-if="!this.men.message" v-for="product in men.slice((currentPage - 1) * itemsPerPage , currentPage * itemsPerPage)">
        ...
        <h1 v-if="this.men.message" class="text-center">{{ men.message }}</h1>
    </div>
</div>
<div class="buttons">
    <hr>
...
computed : {
    totalPages(){
        return Math.ceil(this.men.length / this.itemsPerPage)
    },
    authenticated(){
...
beforeMount() {
    axios.get("http://127.0.0.1:8000/api/Men").then((response)=> {
            if(response.data.message){
            this.men = response.data;
            }
            else{
            this.men = response.data.products;
            }
        }).catch(function(error) {
            console.log(error.response.data.message);
        });
